...
angular.module('xc.indexedDB', []).provider('$indexedDB', function() {
    ...
    module.$get = ['$q', '$rootScope', function($q, $rootScope) {
        ...
        ObjectStore.prototype = {
            ...
            /**
             * Added to indexeddb.js library as it fits best here,
             * although it's not part of the original code.
             *
             * @param idx Name of index
             * @param term Search for values matching 'term'
             * @returns {*}
             */
            "getAllFromIndex": function(idx, term) {
                var results = [], d = $q.defer();
                return this.internalObjectStore(this.storeName, READONLY).then(function(store){
                    var req = store.index(idx).openCursor(IDBKeyRange.bound(term, term + '\uffff'));
                    req.onsuccess = function(e) {
                        var cursor = e.target.result;
                        if(cursor){
            ...
            "each": function(options){
                ...
                return this.internalObjectStore(this.storeName, READWRITE).then(function(store){
                   ...
                   if(options.useIndex) {
                       console.log("index: " + store.index('id'));
